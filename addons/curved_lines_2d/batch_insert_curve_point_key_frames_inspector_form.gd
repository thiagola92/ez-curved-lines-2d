...
func _on_key_frame_capabilities_changed() -> void:
	...
	if not is_instance_valid(GlobalSelection.animation_under_edit_button):
		return
	if GlobalSelection.animation_under_edit_button.get_selected_id() < 0:
		return
	if not is_instance_valid(GlobalSelection.animation_player):
		return
...
func _on_batch_insert_button_pressed() -> void:
	GlobalSelection.batch_insert_key_frames(scalable_vector_shape_2d)
